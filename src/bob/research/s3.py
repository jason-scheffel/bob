...
"""s3: linear-trend projected bracket hold-to-settlement.

Fit OLS to the last 10 closes, project to the hour end, buy YES/NO on
the bracket containing that target.

Treat this module as frozen. Prefer a new strategy module over editing s3.
"""
    ...
    *,
    ...
    for event in events:
        ...
        for minute in minute_list:
            ...
            window = range(minute - TREND_WINDOW + 1, minute + 1)
            ...
            if slope is None:
                exclusions[minute]["bad_slope"] += 1
                continue
            remaining = 60 - minute
            target = closes[-1] + slope * remaining
