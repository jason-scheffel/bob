...
"""s10: majority-range-persistent current-bracket hold.

Buy YES/NO on the live bracket when at least 60% of minute high/low
ranges over the matched remaining-time horizon stayed inside it.
"""
...
STRATEGY = "s10"
STRATEGY_SUMMARY = "majority-range-persistent current bracket"

DEFAULT_CHECKPOINT_MINUTES = (45, 50, 55)
MIN_CHECKPOINT = 30
MIN_RANGE_DWELL = Decimal("0.60")
...
AbstentionReason = Literal["low_range_dwell"]
...
def range_dwell_fraction(
    bars: Sequence[tuple[Decimal, Decimal]],
    bracket: BracketRow,
) -> Decimal:
    """Fraction of complete high/low ranges contained by ``bracket``."""
    contained = sum(
        price_in_bracket(high, bracket.floor_strike, bracket.cap_strike)
        and price_in_bracket(low, bracket.floor_strike, bracket.cap_strike)
        for high, low in bars
    )
    return Decimal(contained) / Decimal(len(bars))
...
def evaluate(
    ...
    *,
    ...
    """Score majority-range-persistent current-bracket hold accuracy."""
    ...
    for event in events:
        ...
        for minute in minute_list:
            ...
            if reason is not None:
                ...
                continue
            selected = matches[0]
            if range_dwell_fraction(bars, selected) < MIN_RANGE_DWELL:
                abstentions[minute]["low_range_dwell"] += 1
                continue
